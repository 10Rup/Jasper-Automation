...
def generate_jrxml_from_image(image_path):
    img = Image.open(image_path)

    prompt = """
    You are a JasperReports expert.

    Analyze the image and generate JRXML layout code using staticText.
...
def process_images_with_gemini(file_id):
    import os, json
    ...
    for region in metadata["regions"]:
        ...
        jrxml = generate_jrxml_from_image(image_path)
    ...
    output_path = os.path.join(folder, "gemini_components.json")
...
@app.get("/generate-gemini/{file_id}")
def generate_gemini(file_id: str):
    path = process_images_with_gemini(file_id)
